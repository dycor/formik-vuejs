   ...
   <v-container>
      ...
      <v-divider></v-divider>
        <v-card class="elevation-12" style="margin-top: 20px;">
            <Formik :initial-values="{ firstname: user.firstname, lastname: user.lastname, email: user.email, password: ''}" @onSubmit="handleSubmit">
                <v-form>
                <v-card-text>
            ...
            <v-card-actions>
              ...
              <v-spacer></v-spacer>
              <v-btn text color='indigo' dark>Save</v-btn>
            </v-card-actions>
                </v-form>
                </Formik>
          </v-card>
...
export default {
  ...
  methods: {
    handleSubmit({ event, values }) {
      ...
      axios
        .put(`/user/${localStorage.token}`, body)
        .then(response => {
          if (response.status === 200) {
            // eslint-disable-next-line no-console
